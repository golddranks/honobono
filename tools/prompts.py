...
{{
    "characters": [
        {{
            ...
            "other_names": ["他の呼び方1", "他の呼び方2"],
            "description": "その人物が誰なのかの短い説明"
        }}
    ]
}}
"""
    ...
    "properties": {
        "characters": {
            ...
            "items": {
                ...
                "required": ["canonical_name", "other_names", "description"],
...
def char_extract_verify_prompt(
    *,
    body: str,
    names: list[dict],
) -> str:
...
抽出された「今回の登場人物」リストについて、以下の二点を本文のみを根拠に確認してください。
(1) 本文に登場する人物が漏れていないか
(2) 本文に登場しない人物が混じっていないか

「漏れ」(missing) のルール:
- 本文中で行動・発言・思考・知覚している人物が漏れていないか確認する。
- 会話・回想・説明の中で言及された人物（その場にいなくても）が漏れていないか確認する。
- 固有名がなくても、特定の人物として識別できる存在（「店員」「母親」など）が漏れていないか確認する。
- 一人称の語り手を忘れていないか確認する。
- 漏れている人物の呼び名を missing に列挙する。漏れがなければ空配列。

「本文に存在しない人物」(hallucinated) のルール:
- 抽出された「今回の登場人物」のうち、本文に登場も言及もされない人物の canonical_name を hallucinated に列挙する。
- 既存の登場人物・あらすじ・前回の本文などから持ち込まれただけで、今回の本文には現れない人物は hallucinated に入れる。
- 本文に登場・言及がある人物は hallucinated に入れない。
- 値は必ず「今回の登場人物」リストにある canonical_name そのものを使う。

その他:
- 抽出済みの人物の同一性判定（呼び名違いの統合）は今回の検証対象ではない。別ステップで行う。
- complete は missing が空のとき true、それ以外は false。
- hallucination は hallucinated が非空のとき true、それ以外は false。
{names_block}
<本文>
{body}
</本文>
...
{{
    ...
    "hallucination": true または false,
    "missing": ["漏れている人物名", ...],
    "hallucinated": ["本文に存在しない人物の canonical_name", ...]
}}
"""


def extract_verify_schema(names: list[dict]):
    """`hallucinated` items are constrained to the input extract's
    canonical_names — only entries already in our list can be flagged as
    hallucinated. `missing` is open-set (chars we don't know about) so its
    items stay unconstrained strings. `names` is always non-empty in
    practice; if extract returns zero chars that is an upstream bug and
    should crash loudly."""
    cnames = [n["canonical_name"] for n in names]
    return {
        "type": "object",
        "properties": {
            ...
            "hallucinated": {
                "type": "array",
                "items": {"type": "string", "enum": cnames},
                "maxItems": len(cnames),
            },
        },
        "required": ["complete", "hallucination", "missing", "hallucinated"],
        "additionalProperties": False,
    }
